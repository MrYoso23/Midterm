...
parser$add_argument("-grey" ,type="logical",help = "grayscale an image")
parser$add_argument("-angle",type="integer", help = "change the angle of the image")
...
augmented <- function(img,H,W,gry,angle){
  if(gry==TRUE){
    imge <- load.image(img)
    gray <- grayscale(imge)
    gray1 <- save.image(im=gray,file=paste0("Gray_Scale_image.jpg"))
  }
  if(H&&W > 1){
    imge <- load.image(img)
    imgrsze <- resize(imge, size_x = H, size_y = W)
    save.image(im = imgrsze,file=paste0("Resized_Image.jpg"))
  }
  if(angle>1){
    imge <- load.image(img)
    invimg <- imrotate(im=imge,angle)
    invimg1 <- save.image(im=invimg,file=paste0("Inverted_Image.jpg"))
  }
}
#output
